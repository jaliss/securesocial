...
abstract class OAuth2Provider(application: Application) extends IdentityProvider(application) {
  ...
  private def getAccessToken[A](code: String)(implicit request: Request[A]):OAuth2Info = {
    val params = Map(
      ...
      OAuth2Constants.RedirectUri -> Seq(securesocial.core.java.ResolverHandler.getResolver().getAuthenticateUrlAbsolute(providerId,request))
    )
    ...
    request.queryString.get(OAuth2Constants.Code).flatMap(_.headOption) match {
      ...
      case None =>
        ...
        var params = List(
          ...
          (OAuth2Constants.RedirectUri, securesocial.core.java.ResolverHandler.getResolver().getAuthenticateUrlAbsolute(providerId,request)),
