...
You will also have to import default SecureSocial routes

bc. # Import SocialSecure routes
*      /auth              module:securesocial

h2. <a name="securing">Securing a Controller</a>

To restrict access to authenticated users just add @With( SecureSocialLogin.class ) to your controller:
...
To be able to just access the user object without having to force a login screen (for example to show the logout button on a public page), just add @With( SecureSocialNoLogin.class ) instead.
...
Depending on the provider, you might also have to configure the callback url. The callback URL is composed of the route where SecureSocial is imported plus the name of the provider. In our example, the callback URL for twitter would be @http://<my application domain>/auth/twitter@
...
# Logout redirection
#
securesocial.logout.redirect=securesocial.SecureSocial.login

Change this to specify where the user is taken after logging out. By default the user is taken back to the login page.
